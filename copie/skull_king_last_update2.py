...
handler_partie = logging.FileHandler('partie2.log', mode='w')
...
class Joueur:
    def __init__(self, nom, is_agent=False, agent_params=None, numero_agent=None):
    ...
    def init_agent(self):
        agent = model2.Agent(self.agent_params[self.numero_agent]['gamma'],
                                 ...
                                 self.agent_params[self.numero_agent]['n_actions'],
        ...
        model_path = f"models/agent_2_couches_caches_{int(self.nom[-1])}.pth"
        if os.path.exists(model_path):
            agent.load(model_path)
            print(f"Modèle chargé pour {int(self.nom[-1])} depuis {model_path}")
        else:
            print(f"Aucun modèle trouvé pour {int(self.nom[-1])}, initialisation avec des poids par défaut.")
        return agent
...
class Partie:
    ...
    def init_joueur(self, nom_joueurs):
        ...
        for nom in nom_joueurs:
            if nom.startswith("agent"):
                joueurs.append(Joueur(nom, is_agent=True, agent_params=self.agent_params, numero_agent=int(nom[-1])))
            else:
    ...
    def predire_plis(self, joueur):
        ...
        if self.choix_nb_pli_aleatoire:
            ...
            while choix_prediction is None:
                ...
                choix_prediction = joueur.agent.choose_action(state, nb_prediction_max=self.tour) - 10 # c'est l'indice de prédiction qu'on cherche entre 0 et 10 or entre 10 et 20 dans le modèle
                ...
                if choix_prediction > self.tour or choix_prediction < 0:
                    # print(f"ta fais de la D pour predire, tu as choisi {choix_prediction}")
                    ...
                    joueur.agent.store_transition(state, choix_prediction+10, state_) # +10 pour prédiction
                    ...
                    state_ = self.obtenir_nouvel_etat(joueur, indice_carte_dispo)
                    joueur.agent.store_transition(state, choix_prediction+10, state_)
                    print(f"{joueur.nom}")
    ...
    def obtenir_etat_actuel(self, joueur, indice_carte_dispo):
        ...
        for carte in joueur.main:
            ...
            caracteristique = dictionnaire_caracteristiques.get(carte.couleur, 0)  # 0 par défaut si inconnu
            cartes_joueur.append(valeur)
            cartes_joueur.append(caracteristique)

        # Compléter avec des zéros si moins de 10 cartes (chaque carte a maintenant 2 positions)
        cartes_joueur += [0, 0] * (10 - len(joueur.main))
        ...
        # 3. Nombre de cartes dans la main
        ...
        for joueur_pli, carte in self.plis:
            valeur = dictionnaire_cartes_speciales.get(carte.valeur, carte.valeur)
            caracteristique = dictionnaire_caracteristiques.get(carte.couleur, 0)
            cartes_pli.append(valeur)
            cartes_pli.append(caracteristique)

        # Compléter avec des zéros si moins de 10 cartes
        cartes_pli += [0, 0] * (10 - len(cartes_pli) // 2)
        ...
        # 10. Choix Tigresse
        choix_tigresse = 0  # Pas encore défini
        if self.carte_tigresse:
            if self.choix_tigresse == "P":
                choix_tigresse = 1
            elif self.choix_tigresse == "D":
                choix_tigresse = 2
        state.append(choix_tigresse)

        return state


    def obtenir_nouvel_etat(self, joueur, indice_carte_dispo):
        dictionnaire_caracteristiques = {
            "rouge": 1,
            "bleu": 2,
            "vert": 3,
        ...
        for joueur in joueurs_tour:
            ...
            while carte_jouee is None:
                ...
                if index is not None:
                    ...
                    if carte == Carte("Tigresse", "spécial"):
    ...
    def jouer(self):
        ...
        reward = 5
        print(f"{gagnant.nom}")
        gagnant.agent.learn_tout(reward)
    ...
    def entrainement_ia(self):
        ...
        for joueur in self.joueurs:
            if joueur.is_agent:
                model_path = os.path.join(model_dir, f'agent_2_couches_caches_{int(joueur.nom[-1])}.pth')
                joueur.agent.save(model_path)
